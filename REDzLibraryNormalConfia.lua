...
local function ApplyBackground(mainFrame, backgroundType, imageId, imageSeq, imageSeqSpeed, lastImageStop)
        backgroundType = backgroundType or "Gradient"
        imageSeqSpeed = imageSeqSpeed or 2
        lastImageStop = lastImageStop ~= false
        
        if backgroundType == "Gradient" then
                Make("Gradient", mainFrame, {
                        Rotation = 45
                })
        elseif backgroundType == "Image" and imageId then
                mainFrame.BackgroundImage = "rbxassetid://" .. tostring(imageId)
                mainFrame.BackgroundTransparency = 0
        elseif backgroundType == "ImageSeq" and imageSeq and #imageSeq > 0 then
                local currentIndex = 1
                local imageSequence = imageSeq
                
                mainFrame.BackgroundImage = "rbxassetid://" .. tostring(imageSequence[currentIndex])
                mainFrame.BackgroundTransparency = 0
                
                local imageChangeConnection
                imageChangeConnection = RunService.Heartbeat:Connect(function()
                        if not mainFrame.Parent then
                                imageChangeConnection:Disconnect()
                                return
                        end
                        
                        imageSeqSpeed = math.max(0.1, imageSeqSpeed)
                        local changeInterval = imageSeqSpeed
                        local timeSinceLastChange = (tick() % (changeInterval * #imageSequence)) / changeInterval
                        local newIndex = math.floor(timeSinceLastChange) + 1
                        
                        if newIndex > #imageSequence then
                                if lastImageStop then
                                        newIndex = #imageSequence
                                        if currentIndex ~= newIndex then
                                                currentIndex = newIndex
                                                mainFrame.BackgroundImage = "rbxassetid://" .. tostring(imageSequence[newIndex])
                                        end
                                        imageChangeConnection:Disconnect()
                                else
                                        newIndex = ((newIndex - 1) % #imageSequence) + 1
                                        if currentIndex ~= newIndex then
                                                currentIndex = newIndex
                                                mainFrame.BackgroundImage = "rbxassetid://" .. tostring(imageSequence[newIndex])
                                        end
                                end
                        elseif currentIndex ~= newIndex then
                                currentIndex = newIndex
                                mainFrame.BackgroundImage = "rbxassetid://" .. tostring(imageSequence[newIndex])
                        end
                end)
...
function redzlib:MakeWindow(Configs)
        ...
        local BackgroundType = Configs.BackgroundType or "Gradient"
        local BackgroundImage = Configs.Image
        local BackgroundImageSeq = Configs.ImageSeq
        local ImageSeqSpeed = Configs.ImageSeqSpeed or 2
        local LastImageStop = Configs.LastImageStop or false
        ...
        local MainFrame = InsertTheme(Create("ImageButton", ScreenGui, {
                ...
                BackgroundTransparency = (BackgroundType == "Gradient" and 0.03 or 0),
                Name = "Hub"
        }), "Main")
        ApplyBackground(MainFrame, BackgroundType, BackgroundImage, BackgroundImageSeq, ImageSeqSpeed, LastImageStop)
        MakeDrag(MainFrame)
